...
predictor variable, we first noticed that `Fr` only takes on 0.052, 0.3 and Inf
in our training and testing data set, and directly using it is not viable since
it contains infinity. Since `Fr`\<1 corresponds with a subcritical flow while
`Fr` \> 1 corresponds with a super critical flow, we create a new categorical
variable `flow` by the following:
