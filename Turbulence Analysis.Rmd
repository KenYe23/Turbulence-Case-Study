...
We explore the simple linear regression model first, which results in an
adjusted R-squared value of 0.9251.
...
The plot below suggests a possible interaction effect between `St` and `flow`
number for the 1st Moment, as the low flow (triangle shape) group reveals a
positive slope, whereas the moderate flow (square shape) and high flow (circular
shape) groups reveal close-to-zero slopes:
...
Applying this insight into our linear model for 1st moment, we add the
interaction term `flow:St`, resulting a higher adjusted R-squared of 0.9676.

```{r result = "hide"}
lm2 <- lm(R_moment_1 ~ gravity + St + flow + flow:St, data = train)
summary(lm2)
```

## Explore Polynomial

We also tried using polynomial terms up to degree of 5 for stokes number, and
...
## Model 1: Linear Regression w/ Interaction Term
